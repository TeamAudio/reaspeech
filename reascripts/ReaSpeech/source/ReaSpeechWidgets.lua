...
ReaSpeechCheckbox.new = function(options)
    ...
    options.changed_handler = options.changed_handler or function(_)
    end
...
ReaSpeechCheckbox.simple = function(default_value, label, changed_handler)
    return ReaSpeechCheckbox.new {
        ...
        label_short = label,
        width_threshold = 0,
        changed_handler = changed_handler or function()
        end
    }
end
...
ReaSpeechCombo.renderer = function(self)
    ...
    local imgui_label = ("##%s"):format(options.label)
    local item_label = options.item_labels[self:value()] or self:value()
    ...
    if ImGui.BeginCombo(self.ctx, imgui_label, item_label, combo_flags) then
        Trap(function()
            for _, item in pairs(options.items) do
                ...
                local display_label = options.item_labels[item] or item
                if ImGui.Selectable(self.ctx, display_label, is_selected) then
                    self:set(item)
                end
            end
        end)
